...
def addNewStock (simbolo, nombre):
    ...
    cursor = conn.cursor()
    cursor.execute("INSERT INTO VALORES (Simbolo, Nombre, Estado) VALUES ("+
                qmarks(simbolo)+", "+qmarks(nombre)+", DEFAULT);")
...
def modifyPost(titulo, articulo):
    ...
    cursor = conn.cursor()
    cursor.execute("UPDATE BLOG SET Titulo="+qmarks(titulo)+", Articulo="+qmarks(articulo)+" WHERE Titulo="+qmarks(titulo)+";")
    conn.commit()
    conn.close()
